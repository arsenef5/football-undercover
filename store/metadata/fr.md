# Fiche store — Français

## Nom (30 caractères max)
Football Undercover

## Sous-titre (App Store, 30 max)
Le jeu de bluff 100 % foot

## Description courte (Google Play, 80 max)
Un mot secret, un imposteur, un carton blanc : démasque-les entre potes.

## Description

Football Undercover, c'est le jeu de soirée où chacun reçoit un mot secret de foot… sauf que certains n'ont pas le même mot, et qu'un joueur n'a rien du tout.

Un seul téléphone, 3 à 20 joueurs, zéro compte, zéro connexion.

COMMENT ÇA MARCHE
• Chacun regarde son rôle en secret sur la carte.
• Les Titulaires ont le mot principal (Neymar), l'Undercover a un mot proche (Vinícius Jr), le Carton blanc n'a aucun mot.
• À tour de rôle, on décrit son mot en une phrase, sans le dire.
• On débat, on vote, on élimine. Le rôle est révélé.
• Le Carton blanc éliminé a une dernière chance : deviner le mot.

200 MOTS DE BASE, EN FRANÇAIS ET EN ANGLAIS
Joueurs de légende et stars d'aujourd'hui, trophées, stades, compétitions, moments légendaires (le coup de boule de Zidane, le triplé de Lucas Moura contre l'Ajax, les 5 buts de Lewandowski en 9 minutes…), memes et styles de jeu. Règle les curseurs pour jouer que des joueurs, ou un peu de tout.

TOUT SE RÈGLE
• Nombre d'undercovers, carton blanc ou non, chrono de discussion.
• Équipes enregistrées, créées automatiquement à chaque partie.
• Points façon Undercover : titulaire +2, undercover +10, carton blanc +6. Classement par session et au total.
• Langue des menus et langue des mots, chacune en français ou en anglais.

VERSION PRO
Un achat unique : zéro publicité et des mots en plus.

Plus qu'un jeu. Discrétion, bluff, légende.

## Mots-clés (App Store, 100 caractères max, séparés par des virgules)
undercover,foot,football,soirée,bluff,jeu de société,imposteur,entre amis,mot secret,party game

## Nouveautés (version 1.0)
Première version : 200 mots foot en FR et EN, équipes, classement, version Pro sans pub.

## Catégories
App Store : Jeux > Soirée (secondaire : Jeux > Mots)
Google Play : Jeux > Jeux de société / Soirée

## Classification
Tout public / PEGI 3 — pas de contenu sensible. Publicités présentes dans la version gratuite (à déclarer).

## URL de la politique de confidentialité
À héberger (GitHub Pages ou ton site) depuis `public/privacy.html`, par exemple :
https://TON-COMPTE.github.io/football-undercover/privacy.html

## Déclaration « App Privacy » (App Store) / « Data safety » (Google Play)
Version gratuite : données liées à l'appareil collectées par AdMob (identifiant publicitaire, données d'usage
publicitaire) — finalité « publicité tierce », avec consentement. Achat intégré : identifiant anonyme et reçus
d'achat via RevenueCat — finalité « fonctionnement de l'app ». Aucune donnée personnelle directe (nom, e-mail,
position). Aucun suivi hors consentement ATT sur iOS.
